...
@section('content')
    ...
    <div class="row">
		...
		<div class="page-title-right">
                    <a href="{{ route('cms.pages.builder.create') }}" class="btn btn-primary">
                        <i class="mdi mdi-view-dashboard-variant"></i> {{ __('Page builder') }}
                    </a>
                    <a href="{{ route('cms.pages.create') }}" class="btn btn-outline-primary">
...
$(document).ready(function() {
    ...
    $(document).on('change', '.toggle-status', function() {
        ...
        $.ajax({
            url: "{{ url('admin/cms/pages') }}/" + id + "/toggle-status",
            type: 'POST',
            data: { _token: '{{ csrf_token() }}' },
            success: function(response) {
                if (response.success) {
