		...
		<div class="card-header d-flex justify-content-between align-items-center gap-2">
			<div class="d-flex align-items-center gap-2 flex-grow-1 min-w-0">
				<button type="button" class="btn btn-sm btn-light border px-1 py-0 flex-shrink-0"
					data-bs-toggle="collapse" data-bs-target="#section-collapse-{{ $sidx }}"
					aria-expanded="true" aria-controls="section-collapse-{{ $sidx }}"
					data-role="section-collapse-toggle"
					title="{{ __('Toggle section') }}">
					<i class="mdi mdi-chevron-down"></i>
				</button>
				<strong class="text-truncate">{{ __('Section') }}</strong>
				@if($section)
			...
			<button type="button" class="btn btn-sm btn-outline-danger flex-shrink-0"
				data-role="remove-section">{{ __('Remove section') }}</button>
		</div>
		...
		<div class="card-body">
			...
			<div class="row g-2 mb-2">
				...
				<div class="col-md-2">
					...
					@php
						$sectionTypeCanonical = ['default', 'hero', 'features', 'about-us', 'services', 'why-choose-us', 'download-app', 'gallery', 'testimonial', 'content', 'cta'];
						if (($pageSlug ?? '') === 'subscription') {
							$sectionTypeCanonical = array_values(array_unique(array_merge(['subscription', 'checkout'], $sectionTypeCanonical)));
						}
						$tp = trim((string) old('sections.'.$sidx.'.type', $section?->type ?? 'default'));
						if ($tp === '') {
							$tp = 'default';
						}
					...
					@endphp
					<select class="form-select" name="sections[{{ $sidx }}][type]">
				...
				<div class="col-md-2">
					...
					<div
						...
						<label class="form-label mb-0"
							for="{{ $sectionActiveId }}">{{ __('Active') }}</label>
			...
			<p class="text-muted small mb-1">{{ __('Section translations') }}</p>
			...
			<div class="tab-content">
				@foreach($languages as $index => $lang)
				@php
				$tr = $section ? $section->translations->where('locale', $lang->code)->first() :
				null;
				@endphp
				<div class="tab-pane fade {{ $index === 0 ? 'show active' : '' }}"
					id="{{ $sectionTabPrefix }}-pane-{{ $lang->code }}" role="tabpanel"
					aria-labelledby="{{ $sectionTabPrefix }}-tab-{{ $lang->code }}">
					<div class="row g-2 mb-2">
						<div class="col-md-4">
							<label class="form-label small mb-0">{{ __('Title') }}
								({{ $lang->code }})</label>
							<input type="text"
						...
						<div class="col-md-4">
							<label
								class="form-label small mb-0">{{ __('Subtitle') }}</label>
							<input type="text"
								class="form-control form-control-sm"
						...
						<div class="col-12">
							<label
								class="form-label small mb-0">{{ __('Description') }}</label>
							...
							'inputId' => 'section-desc-'.$sidx.'-'.$lang->code,
							'inputName' => 'sections['.$sidx.'][translations]['.$lang->code.'][description]',
							'label' => __('Description') . ' (' . $lang->name . ')',
							'value' => old("sections.{$sidx}.translations.{$lang->code}.description", $tr?->description ?? ''),
							'direction' => $lang->direction,
							'placeholder' => __('Enter description...'),
							'tabPaneId' => $sectionTabPrefix.'-pane-'.$lang->code,
							])
						</div>
					</div>
				</div>
			...
			'existingImages' => ($section ?? null) ? $section->getMedia('gallery') : collect([]),
			])
			<hr>
			<div class="d-flex justify-content-between align-items-center mb-2">
				<span class="fw-semibold">{{ __('Section links') }}</span>
				<button type="button" class="btn btn-sm btn-outline-primary"
					data-role="add-section-link">{{ __('Add link') }}</button>
			</div>
			...
			<div class="d-flex justify-content-between align-items-center mb-2">
				...
				<button type="button" class="btn btn-sm btn-primary"
					data-role="add-item">{{ __('Add item') }}</button>
			</div>
